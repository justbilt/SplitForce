    ...
    int       nCmdShow)
{
    ...
    return AnglePlayer::createAndRun();
}

int AnglePlayer::createAndRun(void)
{
    AnglePlayer *host = AnglePlayer::sharedInstance();
    int ret = host->run();
    AnglePlayer::purgeSharedInstance();
    return ret;
}

AnglePlayer *AnglePlayer::s_sharedInstance = NULL;

AnglePlayer *AnglePlayer::sharedInstance(void)
{
    if (!s_sharedInstance)
    {
        s_sharedInstance = new AnglePlayer();
    }
    return s_sharedInstance;
}

void AnglePlayer::purgeSharedInstance(void)
{
    if (s_sharedInstance)
    {
...
int AnglePlayer::run(void)
{
    ...
    do
    {
        ...
        SetWindowTextA(m_hwnd, "angle-player");
...
void AnglePlayer::loadProjectConfig(void)
{
    vector<string> args;
...
void AnglePlayer::createViewMenu(void)
{
...
void AnglePlayer::updateMenu(void)
{
    HMENU menu = GetMenu(m_hwnd);
    HMENU viewMenu = GetSubMenu(menu, 1);
...
void AnglePlayer::updateWindowTitle(void)
{
}

void AnglePlayer::relaunch(void)
{
...
void AnglePlayer::writeDebugLog(const char *log)
{
...
// menu callback
void AnglePlayer::onFileOpenProject(void)
{
...
void AnglePlayer::onFileCreateProjectShortcut(void)
{
    ...
    if (SUCCEEDED(hres))
    {
        ...
        psl->SetDescription(L"AnglePlayer");
...
void AnglePlayer::onFileProjectConfig(void)
{
...
void AnglePlayer::onFileRelaunch(void)
{
    relaunch();
}

void AnglePlayer::onFileExit(void)
{
    DestroyWindow(m_hwnd);
}

void AnglePlayer::onViewChangeFrameSize(int viewMenuID)
{
...
void AnglePlayer::onViewChangeOrientation(int viewMenuID)
{
    bool isLandscape = m_project.isLandscapeFrame();
...
void AnglePlayer::onViewChangeZoom(int scaleMode)
{
    float scale = 1.0f;
...
void AnglePlayer::onHelpAbout(void)
{
    DialogBox(GetModuleHandle(NULL), MAKEINTRESOURCE(IDD_ABOUTBOX), m_hwnd, AboutDialogCallback);
}

// windows callback
LRESULT AnglePlayer::WindowProc(UINT message, WPARAM wParam, LPARAM lParam, BOOL* pProcessed)
{
    int wmId, wmEvent;
    AnglePlayer *host = AnglePlayer::sharedInstance();
...
INT_PTR CALLBACK AnglePlayer::AboutDialogCallback(HWND hDlg, UINT message, WPARAM wParam, LPARAM lParam)
{
